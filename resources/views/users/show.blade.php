...
<div class="relative px-4 pb-32 pt-8 max-w-5xl mx-auto">
    ...
    <section class="mb-16 flex flex-col items-center">
        <div class="relative w-64 h-64 md:w-80 md:h-80 flex items-center justify-center">
            ...
            <div class="absolute -top-4 -right-4 bg-surface-container-high px-4 py-2 rounded-xl border border-secondary/20 shadow-xl backdrop-blur-md">
                ...
                <p class="font-headline text-sm font-bold text-secondary-container">
                    {{ $user->email_verified_at ? 'ACTIVE_LINK' : 'PENDING_LINK' }}
                </p>
            </div>
        </div>
        ...
        <div class="mt-8 text-center">
            ...
            <div class="flex items-center justify-center gap-3">
                ...
                <p class="font-label text-zinc-500 tracking-[0.3em] text-xs uppercase">Elite User • User Archive</p>
            </div>
        </div>
    </section>
    ...
    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
        <div class="md:col-span-2 bg-surface-container-high/40 backdrop-blur-xl p-8 rounded-xl shadow-2xl relative overflow-hidden group">
            ...
            <div class="grid grid-cols-2 gap-8">
                <div>
                    <p class="text-zinc-500 text-[10px] font-headline uppercase tracking-widest mb-1">Archive ID</p>
                    <p class="font-headline text-lg font-bold text-on-surface">{{ $identifier }}</p>
                </div>
                <div>
                    <p class="text-zinc-500 text-[10px] font-headline uppercase tracking-widest mb-1">Clearance Level</p>
                    <p class="font-headline text-lg font-bold text-tertiary">
                        {{ $user->authority_level ? strtoupper(str_replace('_', ' ', $user->authority_level)) : 'REVIEW_REQUIRED' }}
                    </p>
                </div>
                <div>
                    <p class="text-zinc-500 text-[10px] font-headline uppercase tracking-widest mb-1">Created At</p>
                    <p class="font-headline text-lg font-bold text-on-surface">{{ optional($user->created_at)->format('d.m.Y') ?? '—' }}</p>
                </div>
                <div>
                    <p class="text-zinc-500 text-[10px] font-headline uppercase tracking-widest mb-1">Last Sync</p>
                    <p class="font-headline text-lg font-bold text-on-surface">{{ optional($user->updated_at)->diffForHumans() ?? '—' }}</p>
                </div>
                <div class="md:col-span-2">
                    <p class="text-zinc-500 text-[10px] font-headline uppercase tracking-widest mb-1">Archive Summary</p>
                    <p class="text-sm leading-relaxed text-on-surface-variant">{{ $user->summary ?? 'No summary provided.' }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="bg-surface-container-low/60 backdrop-blur-md p-8 rounded-xl border border-secondary/10 flex flex-col justify-between">
            <div>
                <h3 class="font-headline text-zinc-500 text-xs tracking-widest uppercase mb-4">Neural Link</h3>
                <div class="space-y-4">
                    <div class="flex justify-between items-end">
                        <span class="text-xs text-zinc-400">Sync Stability</span>
            ...
            <div class="mt-8">
                <p class="text-xs text-zinc-500 leading-relaxed italic">
                    "Loyalty to the archive is encoded into every user record."
                </p>
            </div>
        </div>
        ...
        <div class="md:col-span-3 bg-surface-container-high/20 backdrop-blur-lg rounded-xl overflow-hidden border border-white/5">
            <div class="px-8 py-4 bg-white/5 flex justify-between items-center">
                <h3 class="font-headline text-xs tracking-widest text-zinc-400 uppercase">Access Records</h3>
            ...
            <div class="p-4 overflow-x-auto">
                <table class="w-full text-left font-body text-sm">
                    ...
                    <tr class="text-zinc-500 border-b border-white/5">
                        <th class="pb-4 font-medium uppercase tracking-tighter text-[10px]">Vector</th>
                        <th class="pb-4 font-medium uppercase tracking-tighter text-[10px]">Operation</th>
                        <th class="pb-4 font-medium uppercase tracking-tighter text-[10px]">Stardate</th>
                        <th class="pb-4 font-medium uppercase tracking-tighter text-[10px] text-right">Metric</th>
                    </tr>
                    </thead>
                    <tbody class="divide-y divide-white/5">
                    <tr class="hover:bg-white/5 transition-colors group">
                        <td class="py-4 font-headline text-on-surface">USERS_CORE</td>
                        <td class="py-4 text-zinc-400">Profile Synthesis</td>
                    ...
                    <tr class="hover:bg-white/5 transition-colors group">
                        ...
                        <td class="py-4 text-zinc-400">Data Re-Index</td>
    ...
    <div class="flex flex-col md:flex-row gap-6 items-center justify-center">
        ...
        <form method="POST" action="{{ route('users.destroy', $user) }}" onsubmit="return confirm('Archive this user?')" class="w-full md:w-auto">
            @csrf
            @method('DELETE')
